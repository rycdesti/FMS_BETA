        ...
        <b-form @submit.prevent="formSubmit" @keydown="form.onKeydown($event)">
            <fieldset class="border p-2">
                ...
                <b-form-group
                        ...
                        description="Please select withholding tax.">
                    <b-form-radio-group v-model="form.voucher.withholding_tax_id"
                                        :options="withholding_tax_opt"
                                        :class="{ 'is-invalid': form.errors.has('voucher.withholding_tax_id') }"
                                        stacked>
                    </b-form-radio-group>
                </b-form-group>
                <has-error :form="form" field="name"/>
    ...
    export default {
        ...
        mounted() {
            ...
            axios.all([
                ...
                axios.get('/api/ap/utils/get_withholding_taxes'),
                axios.get('/api/ap/utils/get_document_type'),
                axios.get('/api/financial/utils/get_chart_account'),
            ]).then(axios.spread(function (bank, withholding_tax, document_type, chart_account) {
                component.bank_opt = bank.data;
                component.withholding_tax_opt = withholding_tax.data;
            ...
            formSubmit: async function () {
                ...
                if (result.value) {
                    const response = is_update ?
                        await this.form.patch(`/api/ap/bank-deposit/${this.form.voucher.id}`) : await this.form.post('/api/ap/bank-deposit');
